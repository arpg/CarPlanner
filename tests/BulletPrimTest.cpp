 ...
 *
 * Usage: bullet_prim -mesh <meshfile.ply> -params <params.csv> -waypoints <waypoints.csv>
 */
...
int main( int argc, char* argv[] ) 
{
  ...
  if(FLAGS_mesh.empty() || FLAGS_params.empty() || FLAGS_waypoints.empty()) {
    LOG(FATAL) << "One of the files(mesh, params, waypoints) missing \n"
        << "Usage: bprim -mesh <meshfile.ply> -params <params.csv> -waypoints <waypoints.yaml>"<<endl;
    return -1;
  }
