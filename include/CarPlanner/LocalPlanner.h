...
struct LocalProblemSolution
{
    ...
    LocalProblemSolution(const MotionSample& sample,
                         const Eigen::Vector5f& dOptParams,
...
struct LocalProblem
{
    void Reset()
    {
        ...
        m_dCoefs = Eigen::Vector4f::Zero();
        m_dStartTorques = Eigen::Vector3f::Zero();
    ...
    Eigen::Vector6f m_dStartPose;   //< Starting 2D pose for the boundary value solver, which is parametrized as [x,y,theta,curvature,v]'
    Eigen::Vector6f m_dGoalPose;    //< Goal 2D pose for the boundary value solver, which is parametrized as [x,y,theta,curvature,v]'
    Sophus::SE3f m_dTinv;
    Sophus::SE3f m_dT3dInv;
    Sophus::SE3f m_dT3d;
    ...
    Eigen::Vector5f m_dInitOptParams;
    Eigen::Vector6f m_dTransformedGoal;
    ...
    Eigen::Vector6fAlignedVec m_vTransformedTrajectory;
    //double m_dMinTrajectoryTime;

    //double m_dDistanceDelta;
    Eigen::Vector4f m_dCoefs;
    bool m_bInertialControlActive;
    double m_dTorqueStartTime;
    double m_dTorqueDuration;
    Eigen::Vector3f m_dStartTorques;

    void UpdateOptParams(const Eigen::VectorXf& dOptParams)
    {
...
public:
    ...
    void SamplePath(const LocalProblem &problem, Eigen::Vector3fAlignedVec &vSamples, bool bBestSolution = false);
    /// Given the local problem struct, will simulate the vehicle physics and produce a motion sample
    Eigen::Vector6f SimulateTrajectory(MotionSample& sample,     //< The motion sample which will be filled by the function
    ...
    Eigen::VectorXf _CalculateSampleError(LocalProblem& problem, double& dMinTrajTime) const { return _CalculateSampleError(problem.m_CurrentSolution.m_Sample,problem,dMinTrajTime); }
    Eigen::VectorXf _CalculateSampleError(const MotionSample &sample, LocalProblem &problem, double &dMinTrajTime) const;
    Eigen::VectorXf _GetWeightVector(const LocalProblem& problem);
    double _CalculateErrorNorm(const LocalProblem &problem, const Eigen::VectorXf& dError);
...
private:
    ...
    bool _CalculateJacobian(LocalProblem &problem,          //< The problem struct defining the current trajectory and goals
                              Eigen::VectorXf& dCurrentErrorVec,          //< This is the current error vector
                              LocalProblemSolution& coordinateDescent,
                              Eigen::MatrixXf &J                    //< Output: The jacobian matrix
                              );
    ...
    Eigen::Vector6f _TransformGoalPose(const Eigen::Vector6f &dGoalPose, const LocalProblem& problem) const;
    /// Returns the trajectory error given the trajectory and a transformed trajectory and end pose
    Eigen::VectorXf _GetTrajectoryError(const MotionSample& sample, const Eigen::Vector6fAlignedVec& vTransformedPoses, const Eigen::Vector6f& endPose, double &dMinTime) const;
    /// Transforms a vehicle state so that it is on the 2D manifold specified by the problem struct
    Eigen::Vector6f _Transform3dGoalPose(const VehicleState& state, const LocalProblem &problem) const;
    ...
    Eigen::MatrixXf& m_dPointWeight;                                       //< The matrix which holds the weighted Gauss-Newton weights
    Eigen::MatrixXf& m_dTrajWeight;
    int m_nPlanCounter;
};
